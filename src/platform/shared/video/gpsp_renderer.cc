...
typedef struct {
    u16 pad0[OAM_DATA_COUNT_AFFINE - 1];
    u16 dx;
    u16 pad1[OAM_DATA_COUNT_AFFINE - 1];
    u16 dmx;
    u16 pad2[OAM_DATA_COUNT_AFFINE - 1];
    u16 dy;
    u16 pad3[OAM_DATA_COUNT_AFFINE - 1];
    u16 dmy;
} t_affp;

typedef void (*bitmap_render_function)(u32 start, u32 end, void *dest_ptr, const u16 *pal);
typedef void (*tile_render_function)(u32 layer, u32 start, u32 end, void *dest_ptr, const u16 *pal);

typedef void (*render_function_u16)(u32 start, u32 end, u16 *scanline, u32 enable_flags);
typedef void (*render_function_u32)(u32 start, u32 end, u32 *scanline, u32 enable_flags);

typedef void (*window_render_function)(u16 *scanline, u32 start, u32 end);

static void render_scanline_conditional(u32 start, u32 end, u16 *scanline, u32 enable_flags = 0x3F);
...
static inline u16 color_flags(u32 layer)
{
    ...
    return (((bldcnt >> layer) & 0x01) | // 1st target
...
static inline void rend_part_tile_Nbpp(u32 bg_comb, u32 px_comb, dtype *dest_ptr, u32 start, u32 end, u16 tile, const u8 *tile_base,
                                       int vertical_pixel_flip, const u16 *paltbl)
{
...
template <typename dtype, rendtype rdtype, bool is8bpp, bool isbase, bool hflip>
static inline void render_tile_Nbpp(u32 bg_comb, u32 px_comb, dtype *dest_ptr, u16 tile, const u8 *tile_base, int vertical_pixel_flip,
                                    const u16 *paltbl)
{
    const u8 *tile_ptr = &tile_base[(tile & 0x3FF) * (is8bpp ? 64 : 32)];
    u16 bgcolor = read_palette_color(paltbl, 0);
...
static void render_scanline_text_fast(u32 layer, u32 start, u32 end, void *scanline, const u16 *paltbl)
{
    u32 bg_control = read_ioreg(REG_ADDR_BGxCNT(layer));
    u16 vcount = read_ioreg(REG_ADDR_VCOUNT);
    u32 map_size = (bg_control >> 14) & 0x03;
    u32 map_width = map_widths[map_size];
    u32 hoffset = (start + read_ioreg(REG_ADDR_BGxHOFS(layer))) % 512;
    u32 voffset = (vcount + read_ioreg(REG_ADDR_BGxVOFS(layer))) % 512;
    ...
    u16 *map_base = (u16 *)BG_SCREEN_ADDR(base_block);
    ...
    map_base += (((voffset % 256) / 8) * (map_width / 8));
    ...
    s32 vflip_off
        = is8bpp ? tile_size_8bpp - 2 * vert_pix_offset - tile_width_8bpp : tile_size_4bpp - 2 * vert_pix_offset - tile_width_4bpp;
    ...
    u32 pixel_run = map_width - hoffset;
    ...
    if (tile_hoff) {
        ...
        if (tile & 0x400) // Tile horizontal flip
            ...
            rend_part_tile_Nbpp<stype, rdtype, is8bpp, isbase, false>(bg_comb, px_comb, dest_ptr, tile_hoff, stop, tile, tile_base,
                                                                      vflip_off, paltbl);
    ...
    if (end) {
        ...
        if (tile & 0x400) // Tile horizontal flip
            ...
            rend_part_tile_Nbpp<stype, rdtype, is8bpp, isbase, false>(bg_comb, px_comb, dest_ptr, 0, end, tile, tile_base, vflip_off,
                                                                      paltbl);
    }
}
...
template <typename stype, rendtype rdtype, bool isbase, bool is8bpp>
static void render_scanline_text_mosaic(u32 layer, u32 start, u32 end, void *scanline, const u16 *paltbl)
{
    u32 bg_control = read_ioreg(REG_ADDR_BGxCNT(layer));
    const u32 mosh = (read_ioreg(REG_ADDR_MOSAIC) & 0xF) + 1;
    const u32 mosv = ((read_ioreg(REG_ADDR_MOSAIC) >> 4) & 0xF) + 1;
    u16 vcount = read_ioreg(REG_ADDR_VCOUNT);
    u32 map_size = (bg_control >> 14) & 0x03;
    u32 map_width = map_widths[map_size];
    u32 hoffset = (start + read_ioreg(REG_ADDR_BGxHOFS(layer))) % 512;
    u16 vmosoff = vcount - vcount % mosv;
    u32 voffset = (vmosoff + read_ioreg(REG_ADDR_BGxVOFS(layer))) % 512;
    stype *dest_ptr = ((stype *)scanline) + start;
...
static void render_scanline_text(u32 layer, u32 start, u32 end, void *scanline, const u16 *paltbl)
{
    // Tile mode has 4 and 8 bpp modes.
    u32 bg_control = read_ioreg(REG_ADDR_BGxCNT(layer));
    bool is8bpp = (read_ioreg(REG_ADDR_BGxCNT(layer)) & 0x80);
    const u32 mosamount = read_ioreg(REG_ADDR_MOSAIC) & 0xFF;
    bool has_mosaic = (bg_control & 0x40) && (mosamount != 0);

    if (has_mosaic) {
        if (is8bpp)
            render_scanline_text_mosaic<stype, rdtype, isbase, true>(layer, start, end, scanline, paltbl);
        else
            render_scanline_text_mosaic<stype, rdtype, isbase, false>(layer, start, end, scanline, paltbl);
    } else {
        if (is8bpp)
            render_scanline_text_fast<stype, rdtype, isbase, true>(layer, start, end, scanline, paltbl);
        else
            render_scanline_text_fast<stype, rdtype, isbase, false>(layer, start, end, scanline, paltbl);
    }
}

static inline u8 lookup_pix_8bpp(u32 px, u32 py, const u8 *tile_base, const u8 *map_base, u32 map_size)
{
...
static inline void rend_pix_8bpp(dsttype *dest_ptr, u8 pval, u32 bg_comb, u32 px_comb, const u16 *pal)
{
    // Alhpa mode stacks previous value (unless rendering the first layer)
    if (pval) {
        if (rdtype == FULLCOLOR)
...
template <typename dsttype, rendtype rdtype> static inline void render_bdrop_pixel_8bpp(dsttype *dest_ptr, u32 bg_comb, u16 bgcol)
{
    // Alhpa mode stacks previous value (unless rendering the first layer)
    if (rdtype == FULLCOLOR)
        *dest_ptr = bgcol;
...
typedef void (*affine_render_function)(u32 layer, u32 start, u32 cnt, const u8 *map_base, u32 map_size, const u8 *tile_base, void *dst_ptr,
                                       const u16 *pal);
...
template <typename dtype, rendtype rdtype, bool isbase, bool wrap, bool rotate, bool mosaic>
static inline void render_affine_background(u32 layer, u32 start, u32 cnt, const u8 *map_base, u32 map_size, const u8 *tile_base,
                                            void *dst_ptr_raw, const u16 *pal)
{
    ...
    s32 dx = (s16)read_ioreg(REG_ADDR_BGxPA(layer));
    s32 dy = (s16)read_ioreg(REG_ADDR_BGxPC(layer));
    ...
    const u32 mosh = (mosaic ? (read_ioreg(REG_ADDR_MOSAIC)) & 0xF : 0) + 1;
    ...
    if (wrap) {
        ...
        if (rotate) {
            for (u32 i = 0; cnt; i++, cnt--) {
                ...
                source_x += dx;
                source_y += dy; // Move to the next pixel
            }
        } else {
...
template <typename dsttype, rendtype rdtype, bool isbase>
static void render_scanline_affine(u32 layer, u32 start, u32 end, void *scanline, const u16 *pal)
{
    ...
    // The tilemap base is selected via bgcnt (16KiB chunks)
    u32 tilecntrl = (bg_control >> 2) & 0x03;
    u8 *tile_base = BG_CHAR_ADDR(tilecntrl);

    dsttype *dest_ptr = ((dsttype *)scanline) + start;
    const u32 mosamount = read_ioreg(REG_ADDR_MOSAIC) & 0xFF;

    bool has_mosaic = (bg_control & 0x40) && (mosamount != 0);
    bool has_rotation = read_ioreg(REG_ADDR_BGxPC(layer)) != 0;
    ...
    u32 fidx = (has_wrap ? 0x4 : 0) | (has_rotation ? 0x2 : 0) | (has_mosaic ? 0x1 : 0);
...
static inline void bitmap_pixel_write(buftype *dst_ptr, pixfmt val, const u16 *palptr, u16 px_attr)
{
    if (mode != 4)
...
static inline void render_scanline_bitmap(u32 start, u32 end, void *scanline, const u16 *palptr)
{
    ...
    bool second_frame = (mode >= 4) && (read_ioreg(REG_ADDR_DISPCNT) & 0x10);
...
static inline void render_obj_part_tile_Nbpp(u32 px_comb, dsttype *dest_ptr, u32 start, u32 end, u32 tile_offset, u16 palette,
                                             const u16 *pal)
{
    // Note that the last VRAM bank wrap around, hence the offset aliasing
    const u8 *tile_ptr = OBJ_VRAM0 + (tile_offset & 0x7FFF);
        ...
        for (u32 i = start; i < end; i++, dest_ptr++) {
            ...
            if (pval) {
                if (rdtype == FULLCOLOR)
                    ...
                    *dest_ptr = dest_ptr[DISPLAY_WIDTH];
            }
        }
    } else {
        // Only 32 bits (8 pixels * 4 bits)
...
static void render_object(s32 delta_x, u32 cnt, stype *dst_ptr, u32 tile_offset, u32 px_comb, u16 palette, const u16 *palptr)
{
    ...
    if (delta_x < 0) { // Left part is outside of the screen/window.
        ...
        if (tile_off) {
            ...
            render_obj_part_tile_Nbpp<stype, rdtype, is8bpp, hflip>(px_comb, dst_ptr, tile_off, tile_off + maxpix, tile_offset, palette,
                                                                    palptr);
    ...
    cnt = cnt % 8;
    if (cnt)
        render_obj_part_tile_Nbpp<stype, rdtype, is8bpp, hflip>(px_comb, dst_ptr, 0, cnt, tile_offset, palette, palptr);
}
...
template <typename stype, rendtype rdtype, bool is8bpp, bool hflip>
static void render_object_mosaic(s32 delta_x, u32 cnt, stype *dst_ptr, u32 base_tile_offset, u32 mosh, u32 px_comb, u16 palette,
                                 const u16 *pal)
{
    const u32 tile_bsize = is8bpp ? tile_size_8bpp : tile_size_4bpp;
    const s32 tile_size_off = hflip ? -tile_bsize : tile_bsize;
...
static void render_affine_object(const t_sprite *obji, const t_affp *affp, bool is_double, u32 start, u32 end, stype *dst_ptr, u32 mosv,
                                 u32 mosh, u32 base_tile, u32 pxcomb, u16 palette, const u16 *palptr)
{
    // Tile size in bytes for each mode
    const u32 tile_bsize = is8bpp ? tile_size_8bpp : tile_size_4bpp;
    ...
    bool obj1dmap = read_ioreg(REG_ADDR_DISPCNT) & 0x40;
    ...
    for (u32 i = 0; i < cnt; i++) {
        ...
        if (!mosaic || !(i % mosh)) {
            ...
            if (is8bpp) {
                ...
                const u32 tile_off = base_tile + // Character base
                ...
                pixval = *(OBJ_VRAM0 + (tile_off & 0x7FFF)); // Read pixel value!
            } else {
                const u32 tile_off = base_tile + // Character base
                    ((pixel_y >> 3) * tile_pitch) + // Skip vertical blocks
                    ((pixel_x >> 3) * tile_bsize) + // Skip horizontal blocks
                ...
                u8 pixpair = *(OBJ_VRAM0 + (tile_off & 0x7FFF)); // Read 2 pixels @4bpp
...
inline static void render_sprite(const t_sprite *obji, bool is_affine, u32 start, u32 end, stype *scanline, u32 pxcomb, const u16 *palptr)
{
    s32 vcount = read_ioreg(REG_ADDR_VCOUNT);
    bool obj1dmap = read_ioreg(REG_ADDR_DISPCNT) & 0x40;
    u32 tile = obji->oam_data->split.tileNum;
    if (is8bpp && !obj1dmap) {
        tile &= ~1;
    }
    u32 base_tile = tile * 32;

    const u32 mosv = (mosaic ? (read_ioreg(REG_ADDR_MOSAIC) >> 12) & 0xF : 0) + 1;
    const u32 mosh = (mosaic ? (read_ioreg(REG_ADDR_MOSAIC) >> 8) & 0xF : 0) + 1;
    ...
    u16 pal = (is8bpp ? 0 : (obji->oam_data->split.paletteNum << 4));

    if (is_affine) {
        u32 pnum = obji->oam_data->split.matrixNum;
        const t_affp *affp_base = (t_affp *)OAM;
        const t_affp *affp = &affp_base[pnum];

        if (affp->dy == 0) // No rotation happening (just scale)
            render_affine_object<stype, rdtype, mosaic, is8bpp, false>(obji, affp, obji->is_double, start, end, scanline, mosv, mosh,
                                                                       base_tile, pxcomb, pal, palptr);
        else // Full rotation and scaling
            render_affine_object<stype, rdtype, mosaic, is8bpp, true>(obji, affp, obji->is_double, start, end, scanline, mosv, mosh,
                                                                      base_tile, pxcomb, pal, palptr);
    } else {
        ...
        bool hflip = (obji->oam_data->split.matrixNum >> 3) & 1;
        bool vflip = (obji->oam_data->split.matrixNum >> 4) & 1;
        ...
        const u32 tile_offset = base_tile + // Char offset
        ...
        if (mosaic && mosh > 1) {
            if (hflip)
                render_object_mosaic<stype, rdtype, is8bpp, true>(obj_x_offset, max_draw, &scanline[start], tile_offset, mosh, pxcomb, pal,
                                                                  palptr);
            else
                render_object_mosaic<stype, rdtype, is8bpp, false>(obj_x_offset, max_draw, &scanline[start], tile_offset, mosh, pxcomb, pal,
                                                                   palptr);
        } else {
            if (hflip)
                render_object<stype, rdtype, is8bpp, true>(obj_x_offset, max_draw, &scanline[start], tile_offset, pxcomb, pal, palptr);
            else
                render_object<stype, rdtype, is8bpp, false>(obj_x_offset, max_draw, &scanline[start], tile_offset, pxcomb, pal, palptr);
        }
    }
}
...
template <typename stype, rendtype rdtype> void render_scanline_objs(u32 priority, u32 start, u32 end, void *raw_ptr, const u16 *palptr)
{
    ...
    for (objn = objcnt - 1; objn >= 0; objn--) {
        ...
        const OamData *oam_data = (OamData *)&OAM[objoff * OAM_DATA_SIZE_AFFINE];

        u16 obj_shape = oam_data->split.shape;
        u16 obj_size = oam_data->split.size;
        bool is_affine = oam_data->split.affineMode & 1;
        bool is_trans = oam_data->split.objMode == OBJ_MOD_SEMITRAN;
        ...
        t_sprite obji = {
            ...
            .oam_data = oam_data,
            .is_double = !!((oam_data->split.affineMode >> 1) & 1),
        };
        ...
        bool emosaic = oam_data->split.mosaic;
        bool is_8bpp = oam_data->split.bpp;

        // Some games enable mosaic but set it to size 0 (1), so ignore.
        const u32 mosreg = read_ioreg(REG_ADDR_MOSAIC) & 0xFF00;

        if (emosaic && mosreg) {
            if (is_8bpp)
                render_sprite<stype, rdtype, true, true>(&obji, is_affine, start, end, scanline, pxcomb, palptr);
            else
                render_sprite<stype, rdtype, false, true>(&obji, is_affine, start, end, scanline, pxcomb, palptr);
        } else {
            if (is_8bpp)
                render_sprite<stype, rdtype, true, false>(&obji, is_affine, start, end, scanline, pxcomb, palptr);
            else
                render_sprite<stype, rdtype, false, false>(&obji, is_affine, start, end, scanline, pxcomb, palptr);
        }
    }
}
...
static void order_obj(u32 video_mode)
{
    u32 obj_num;
    u32 row;
    u16 rend_cycles[DISPLAY_HEIGHT];

    bool hblank_free = read_ioreg(REG_ADDR_DISPCNT) & 0x20;
    u16 max_rend_cycles = !sprite_limit ? REND_CYC_MAX : hblank_free ? REND_CYC_REDUCED : REND_CYC_SCANLINE;
    ...
    for (obj_num = 0; obj_num < 128; obj_num++) {
        const OamData *oam_data = (OamData *)&OAM[obj_num * OAM_DATA_SIZE_AFFINE];
        ...
        if ((video_mode >= 3) && (!(oam_data->split.tileNum & 0x200)))
            continue;
...
template <blendtype bldtype, bool st_objs> static void merge_blend(u32 start, u32 end, u16 *dst, u32 *src)
{
    u32 bldalpha = read_ioreg(REG_ADDR_BLDALPHA);
    u32 brightf = MIN(16, read_ioreg(REG_ADDR_BLDY) & 0x1F);
...
template <blendtype bldtype> static void merge_brightness(u32 start, u32 end, u16 *srcdst)
{
    u32 brightness = MIN(16, read_ioreg(REG_ADDR_BLDY) & 0x1F);
...
void tile_render_layers(u32 start, u32 end, dsttype *dst_ptr, u32 enabled_layers)
{
    ...
    bool objlayer_is_1st_tgt = ((read_ioreg(REG_ADDR_BLDCNT) >> 4) & 1) != 0;
    bool has_trans_obj = obj_alpha_count[read_ioreg(REG_ADDR_VCOUNT)];
    ...
    for (lnum = 0; lnum < layer_count; lnum++) {
        ...
        if (is_obj && obj_enabled) {
            ...
            if (objmode == STCKCOLOR && can_skip_blend)
                render_scanline_objs<dsttype, INDXCOLOR>(layer & 0x3, start, end, dst_ptr, &PLTT[0x100]);
            else
                render_scanline_objs<dsttype, objmode>(layer & 0x3, start, end, dst_ptr, &PLTT[0x100]);

            base_done = 1;
        } else if (!is_obj && ((1 << layer) & enabled_layers)) {
            bool layer_is_1st_tgt = ((read_ioreg(REG_ADDR_BLDCNT) >> layer) & 1) != 0;
            ...
            if (bgmode == STCKCOLOR && can_skip_blend) {
                ...
                rdfns[fnidx](layer, start, end, dst_ptr, PLTT);
            } else {
                static const tile_render_function rdfns[4] = {
...
static void render_w_effects(u32 start, u32 end, u16 *scanline, u32 enable_flags, const layer_render_struct *renderers)
{
    bool effects_enabled = enable_flags & 0x20; // Window bit for effects.
    bool obj_blend = obj_alpha_count[read_ioreg(REG_ADDR_VCOUNT)] > 0;
    u16 bldcnt = read_ioreg(REG_ADDR_BLDCNT);

    // If the window bits disable effects, default to NONE
    u32 effect_type = effects_enabled ? ((bldcnt >> 6) & 0x03) : COL_EFFECT_NONE;

    switch (effect_type) {
        case COL_EFFECT_BRIGHT: {
            // If no layers are 1st target, no effect will really happen.
            bool some_1st_tgt = (read_ioreg(REG_ADDR_BLDCNT) & 0x3F) != 0;
            // If the factor is zero, it's the same as "regular" rendering.
            bool non_zero_blend = (read_ioreg(REG_ADDR_BLDY) & 0x1F) != 0;
            if (some_1st_tgt && non_zero_blend) {
                if (obj_blend) {
                    u32 tmp_buf[DISPLAY_WIDTH];
        ...
        case COL_EFFECT_BLEND: {
            ...
            bool some_1st_tgt = (read_ioreg(REG_ADDR_BLDCNT) & 0x003F) != 0;
            bool some_2nd_tgt = (read_ioreg(REG_ADDR_BLDCNT) & 0x3F00) != 0;
            // If 1st target is 100% opacity and 2nd is 0%, just render regularly.
            bool non_trns_tgt = (read_ioreg(REG_ADDR_BLDALPHA) & 0x1F1F) != 0x001F;
...
static const bitmap_layer_render_struct idx32_bmrend[3][2]
    = { bitmap_layer_render_functions(INDXCOLOR, u32, 3, u16, DISPLAY_WIDTH, DISPLAY_HEIGHT),
        bitmap_layer_render_functions(INDXCOLOR, u32, 4, u8, DISPLAY_WIDTH, DISPLAY_HEIGHT),
        bitmap_layer_render_functions(INDXCOLOR, u32, 5, u16, DISPLAY_HEIGHT, 128) };

// Render the BG and OBJ in a bitmap scanline from start to end ONLY if
// enable_flag allows that layer/OBJ.

template <rendtype bgmode, rendtype objmode, typename dsttype>
static void bitmap_render_layers(u32 start, u32 end, dsttype *scanline, u32 enable_flags)
{
    u16 dispcnt = read_ioreg(REG_ADDR_DISPCNT);
    bool has_trans_obj = obj_alpha_count[read_ioreg(REG_ADDR_VCOUNT)];
    bool objlayer_is_1st_tgt = (read_ioreg(REG_ADDR_BLDCNT) & 0x10) != 0;
    bool bg2_is_1st_tgt = (read_ioreg(REG_ADDR_BLDCNT) & 0x4) != 0;

    // Fill in the renderers for a layer based on the mode type,
    static const bitmap_layer_render_struct renderers[3][2]
        = { bitmap_layer_render_functions(bgmode, dsttype, 3, u16, DISPLAY_WIDTH, DISPLAY_HEIGHT),
            bitmap_layer_render_functions(bgmode, dsttype, 4, u8, DISPLAY_WIDTH, DISPLAY_HEIGHT),
            bitmap_layer_render_functions(bgmode, dsttype, 5, u16, DISPLAY_HEIGHT, 128) };

    const u32 mosamount = read_ioreg(REG_ADDR_MOSAIC) & 0xFF;
    u32 bg_control = read_ioreg(REG_ADDR_BG2CNT);
    ...
    for (layer_order_pos = 0; layer_order_pos < layer_count; layer_order_pos++) {
        ...
        if (current_layer & 0x04) {
            if (enable_flags & 0x10) {
                ...
                // Optimization: skip blending mode if no blending can happen to this layer
                if (objmode == STCKCOLOR && can_skip_blend)
                    render_scanline_objs<dsttype, INDXCOLOR>(current_layer & 3, start, end, scanline, &PLTT[0x100]);
                else
                    render_scanline_objs<dsttype, objmode>(current_layer & 3, start, end, scanline, &PLTT[0x100]);
            }
        } else {
            if (enable_flags & 0x04) {
                ...
                const bitmap_layer_render_struct *rd = (bgmode == STCKCOLOR && can_skip_blend) ? idxm_rend : mode_rend;

                if (dy)
                    rd->affine_render(start, end, scanline, PLTT);
                else if (dx == 256)
                    rd->blit_render(start, end, scanline, PLTT);
                else
                    rd->scale_render(start, end, scanline, PLTT);
            }
        }
    }
}
...
// effects are enabled).
static void render_scanline_conditional(u32 start, u32 end, u16 *scanline, u32 enable_flags)
{
    u16 dispcnt = read_ioreg(REG_ADDR_DISPCNT);
...
inline bool in_window_y(u32 vcount, u32 top, u32 bottom)
{
    ...
    if (top > DISPLAY_HEIGHT + 67) // This causes the window to be invisible
        return false;
    if (bottom > DISPLAY_HEIGHT + 67) // This makes it all visible
        return true;
...
template <window_render_function outfn, unsigned winnum> static void render_window_n_pass(u16 *scanline, u32 start, u32 end)
{
    u32 vcount = read_ioreg(REG_ADDR_VCOUNT);
    // Check the Y coordinates to check if they fall in the right row
    u32 win_top = WIN_GET_LOWER(*(winreg_t *)(REG_ADDR_WINxV(winnum)));
    u32 win_bot = WIN_GET_HIGHER(*(winreg_t *)(REG_ADDR_WINxV(winnum)));
    // Check the X coordinates and generate up to three segments
    // Clip the coordinates to the [start, end) range.
    u32 win_lraw = WIN_GET_LOWER(*(winreg_t *)(REG_ADDR_WINxH(winnum)));
    u32 win_rraw = WIN_GET_HIGHER(*(winreg_t *)(REG_ADDR_WINxH(winnum)));
    ...
    if (!in_window_y(vcount, win_top, win_bot) || (win_lraw == win_rraw))
        ...
        u32 winin = (*(winreg_t *)REG_ADDR_WININ) & 0xFFFF;
        ...
        if (goodwin) {
            ...
            // Render the [win_l, DISPLAY_WIDTH] range ("inside")
            if (win_l != end)
...
void update_scanline(void)
{
    ...
    if (video_mode) {
        ...
        const u32 bgmosv = ((read_ioreg(REG_ADDR_MOSAIC) >> 4) & 0xF) + 1;

        if (read_ioreg(REG_ADDR_BG2CNT) & 0x40) { // Mosaic enabled for this BG
            if ((vcount % bgmosv) == bgmosv - 1) { // Correct after the last line
                affine_reference_x[0] += (s16)read_ioreg(REG_ADDR_BG2PB) * bgmosv;
                affine_reference_y[0] += (s16)read_ioreg(REG_ADDR_BG2PD) * bgmosv;
            }
        } else {
        ...
        if (read_ioreg(REG_ADDR_BG3CNT) & 0x40) {
            if ((vcount % bgmosv) == bgmosv - 1) {
                affine_reference_x[1] += (s16)read_ioreg(REG_ADDR_BG3PB) * bgmosv;
                affine_reference_y[1] += (s16)read_ioreg(REG_ADDR_BG3PD) * bgmosv;
            }
        } else {
